...
__all__ = ('tangential_pvd_vs_r_engine', )


@cython.boundscheck(False)
@cython.wraparound(False)
@cython.nonecheck(False)
def tangential_pvd_vs_r_engine(double_mesh, x1in, y1in, z1in, x2in, y2in, z2in,
    ...
    cdef cnp.float64_t[:] vtan_sum  = np.zeros(num_rbins_normalized, dtype=np.float64)
    cdef cnp.float64_t[:] vtansq_sum = np.zeros(num_rbins_normalized, dtype=np.float64)

    # --- sorted position/velocity arrays (identical boilerplate to radial engine) ---
    ...
    cdef cnp.float64_t x2shift, y2shift, z2shift
    cdef cnp.float64_t dx, dy, dz, dvx, dvy, dvz
    cdef cnp.float64_t drsq, rperpsq, rperp, r, normed_drsq
    cdef cnp.float64_t vrad, vlos, vtan, vtansq
    cdef cnp.float64_t x1tmp, y1tmp, z1tmp
    cdef cnp.float64_t vx1tmp, vy1tmp, vz1tmp
    cdef cnp.float64_t distance_norm1tmp
    cdef int Ni, Nj, i, j, k

    cdef cnp.float64_t[:] x_icell1, y_icell1, z_icell1
    cdef cnp.float64_t[:] x_icell2, y_icell2, z_icell2
    cdef cnp.float64_t[:] vx_icell1, vy_icell1, vz_icell1
    cdef cnp.float64_t[:] vx_icell2, vy_icell2, vz_icell2
    ...
    for icell1 in range(first_cell1_element, last_cell1_element):
        ...
        if Ni > 0:
            ...
            for nonPBC_ix2 in range(leftmost_ix2, rightmost_ix2):
                if nonPBC_ix2 < 0:              x2shift = -xperiod*PBCs
                elif nonPBC_ix2 >= num_x2divs:  x2shift = +xperiod*PBCs
                else:                            x2shift = 0.
                ix2 = nonPBC_ix2 % num_x2divs

                for nonPBC_iy2 in range(leftmost_iy2, rightmost_iy2):
                    if nonPBC_iy2 < 0:              y2shift = -yperiod*PBCs
                    elif nonPBC_iy2 >= num_y2divs:  y2shift = +yperiod*PBCs
                    else:                            y2shift = 0.
                    iy2 = nonPBC_iy2 % num_y2divs

                    for nonPBC_iz2 in range(leftmost_iz2, rightmost_iz2):
                        if nonPBC_iz2 < 0:              z2shift = -zperiod*PBCs
                        elif nonPBC_iz2 >= num_z2divs:  z2shift = +zperiod*PBCs
                        else:                            z2shift = 0.
                        ...
                        if Nj > 0:
                            for i in range(0, Ni):
                                ...
                                for j in range(0, Nj):
                                    ...
                                    rperpsq = dx*dx + dy*dy  # los = z

                                    # skip coincident pairs and pairs along los
                                    # (rperp=0 makes vtan undefined)
                                    if drsq == 0 or rperpsq == 0:
                                        pass
                                    else:
                                        ...
                                        if normed_drsq <= rbins_normalized_squared[k]:
                                            r      = c_sqrt(drsq)
                                            rperp  = c_sqrt(rperpsq)
                                            vrad   = (dx*dvx + dy*dvy + dz*dvz) / r
                                            vlos   = dvz
                                            # v_t = (r*v_los - dz*v_r) / r_p
                                            vtan   = (r*vlos - dz*vrad) / rperp
                                            vtansq = vtan * vtan
    ...
    return np.array(counts), np.array(vtan_sum), np.array(vtansq_sum)