"""Generates LOS velocity samples using Sweeney and Rozo 2025 Model, using individual halos' R and M values. 
"""
...
def process_bin(k, i, r_cens, vphyslos_ls_exs, R_ls_exs, rlos_ls_exs, gal_MAP, a, H, h):
    ...
    bin_M = M_ls_exs[index]
    
    # Create samples
    samp_vr_vt = np.array([
        joint_dist_samples(1, r=np.sqrt(bin_R[j]**2 + bin_rlos[j]**2), M=bin_M[j], pars=gal_MAP) 
...
# Prepare arguments
args_list = [
    (k, i, r_cens, vphyslos_ls_exs, R_ls_exs, rlos_ls_exs, gal_MAP, a, H, h)
    for k, M in enumerate(Mvirs) 
    for i, R in enumerate(r_cens)
]
...
# Save results
with h5.File(samp_path+'gal_model_ind_mass_samples_r_4_30.hdf5', 'a') as hdf:
