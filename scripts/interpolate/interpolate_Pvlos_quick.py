...
rlos_edges = np.linspace(-100, 100, 100+1)  # reduced num of bins
...
def P_vphys_R_vec(v_phys, R, M, pars): #newest, with updated integration, normalization. 
    ...
    v_buffer = 500
    vr_e = np.linspace(-vpec_data_max - v_buffer, vpec_data_max + v_buffer, 50+1) #quick implementation. May affect width of Pvlos slightly, but not conclusive. 
...
Mvals = np.logspace(np.log10(0.5), np.log10(11), 100) # grid of Mvals
...
# save results
path = '/spiff/cosweeney/simulations/MDPL2/data/Pvlos_M_grid_norm_quick_rlos_100'
